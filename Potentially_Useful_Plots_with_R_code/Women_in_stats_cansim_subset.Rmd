...
data |> dplyr::filter(GEO == "Canada")|>
  ...
  mutate(Gender = str_replace_all(Gender, pattern = "\n", replacement = ""))|>
  dplyr::filter(Gender != "Total gender")|>
  mutate(Gender = str_replace_all(Gender, pattern = "Non-binary person and Unknown gender", replacement = "Non-binary person and \n Unknown gender"))|>
...
data |> dplyr::filter(GEO == "Canada")|>
  ...
  mutate(Gender = str_replace_all(Gender, pattern = "Non-binary person and Unknown", replacement = "Non-binary person and \n Unknown gender"))|>
  mutate(Gender = factor(Gender,
                       levels = c(
                         "Woman",
                         "Man",
...
data |> dplyr::filter(GEO == "Canada")|>
  ...
  mutate(Gender = str_replace_all(Gender, pattern = "\n", replacement = "")) |> 
  dplyr::filter(Gender != "Total gender")|>
  mutate(Genre = case_match(Gender,
                                   "Non-binary person and Unknown gender" ~ "Personne \n non-binaire et \n genre inconnu",
                                   "Man" ~ "Homme",
                                  "Woman" ~ "Femme"))|>
...
# dplyr::filter(Number.and.median == "Median age of full-time teaching staff")|>
  ggplot()+
  geom_line(aes(x = Date, y = VALUE, colour = Genre))+
  ylab("Nombre")+
...
for (rank_group in c('Full Professor', 'Associate Professor', 'Assistant Professor')){
  ...
  mutate(Gender = str_replace_all(Gender, pattern = "\n", replacement = "")) |> 
    ...
    mutate(Gender = str_replace_all(Gender, pattern = "Non-binary person and Unknown gender", replacement = "Non-binary \n person and \n Unknown gender"))|>
  mutate(Gender = factor(Gender,
                       levels = c(
                         "Woman",
                         "Man",
...
# dplyr::filter(Number.and.median == "Median age of full-time teaching staff")|>
  ...
  ggtitle(paste0("Nombre de ",
         ...
         "Assistant professor" ~ "Professeur(e)s adjoint(e)", 
         "Associate professor" ~ "Professeur(e)s agrégé(e)",
         "Full professor"     ~ "Professeur(e)s titulaire"),
...
data |> dplyr::filter(GEO == "Canada")|>
  ...
  theme_ipsum(plot_title_size = 12,
              ...
              axis_title_size = 12,
              caption_size = 12)+
  ggtitle(paste0( "Diplômé(e)s en mathématiques et statistiques"))  
...
for (prog_group in c('Doctoral', 'Masters', 'Undergraduate')){
  ...
  dplyr::filter(Field.of.study %in% c(
     ...
     mutate(Gender = case_match(Gender,
                             ...
                             "Total, gender" ~ "Total gender"))|>
    dplyr::filter(Gender != "Total gender")|>
    bind_rows(data.frame( Date=as.Date("2012-07-01"),VALUE= NA, Gender = "Non-binary \n person and \n Unknown gender"))|>
      mutate(Gender = factor(Gender,
                       levels = c(
                         "Woman",
                         "Man",
...
for (prog_group in c('doctorat', 'maîtrise', 'baccalauréat')){
  ...
  mutate(Gender = case_match(Gender,
                             "Gender unknown" ~ "Non-binary person and\n Unknown gender",
                             "Man" ~ "Man",
                             "Woman" ~ "Woman",
                             "Total, gender" ~ "Total gender"))|>
  dplyr::filter(Gender != "Total gender")|>
   ...
   mutate(Genre = factor(Genre,
                       levels = c(
                         ...
                         "Personne \n non-binaire et\n genre inconnu")))|>
    dplyr::filter(!Gender %in% c("Total, gender", "NA"))|>
     dplyr::select(Date, VALUE, Gender,Genre)|>
    ggplot()+
    geom_line(aes(x = Date, y = VALUE, colour =  Genre))+
  ylab("Nombre")+
  theme_ipsum(plot_title_size = 12,
              ...
              caption_size = 12)+
    expand_limits(y=0) +
  ggtitle(paste0( "Diplômé(e)s de ",prog_group," en mathématiques et statistiques"))  
